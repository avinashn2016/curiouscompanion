document.addEventListener('DOMContentLoaded', function() {
  const initialQuestions = [
    "Hello! Please answer a few questions so that I can learn about your week and also generate the plan for the week ahead. What's your name?",
  ...
  signupForm.addEventListener('submit', function(e) {
    ...
    fetch('/send-sms', {
      ...
      if (response.ok) {
        ...
        alert('Ran out of trial credits. Please contact Twilio account owner or try again.');
      }
    }).catch(error => {
  ...
  scheduleForm.addEventListener('submit', function(e) {
    ...
    sessionDate.setDate(sessionDate.getDate() + ((7 + day - sessionDate.getDay()) % 7));
    sessionDate.setHours(time.split(':')[0], time.split(':')[1]);

    const sessionTime = sessionDate.toLocaleString();

    appointmentInfoElement.textContent = `${userName}, your next session is scheduled for ${sessionTime}`;
    appointmentDetailsElement.style.display = 'block';

    const confirmationMessage = document.createElement('div');
    confirmationMessage.classList.add('confirmation-message');
    confirmationMessage.textContent = 'Congratulations, your booking is confirmed!!';
    ...
    setTimeout(() => {
      ...
      emailPrompt.classList.add('email-prompt');
      emailPrompt.innerHTML = 'Do you want me to email the booking and remind you? <button id="email-yes">Yes</button> <button id="email-no">No</button>';
      document.body.appendChild(emailPrompt);

      document.getElementById('email-yes').addEventListener('click', () => {
        sendEmailReminder(sessionTime);
        emailPrompt.remove();
      });

      document.getElementById('email-no').addEventListener('click', () => {
        emailPrompt.remove();
      });
    }, 4000);
  });

  function sendEmailReminder(sessionTime) {
    const emailBody = encodeURIComponent(`Hi ${userName},\n\nThis is a reminder for your upcoming Reflective Buddy session on ${sessionTime}.\n\nBest regards,\nReflective Buddy Team`);
